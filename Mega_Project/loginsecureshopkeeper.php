...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_farmer = "SELECT * FROM shopkeepers WHERE username='$username'  AND password='$password'";
        $result_shopkeeper = $con->query($sql_farmer);

        if ($result_shopkeeper->num_rows == 1) {
            // User found, set session variables and redirect to a secure page
            $_SESSION['username'] = $username;
            header("Location: addproduct1.php"); // Change this to your secure page
            exit();
        } else {
